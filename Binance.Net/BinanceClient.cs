...
namespace Binance.Net
{
    ...
    public class BinanceClient : RestClient, IBinanceClient
    {
        ...
        public async Task<BinanceTradeRuleResult> CheckTradeRules(string symbol, decimal quantity, decimal? price, OrderType type)
        {
            ...
            if (symbolData.LotSizeFilter != null || (symbolData.MarketLotSizeFilter != null && type == OrderType.Market))
            {
                ...
                // Commented Code no Need to Make any operation on it
                ...
                if (minQty.HasValue)
                {
                    ...
                    if (stepSize.Value == 1)
                    {
                        quantity = decimal.Floor(quantity);
                    }
                    if (stepSize.Value == 0.01M)
                    {
                        quantity = quantity - (quantity % 0.01M);
                    }
                    if (stepSize.Value == 0.001M)
                    {
                        quantity = quantity - (quantity % 0.001M);
                    }

                    // For USDT
                    if (stepSize.Value == 0.1M)
                    {
                        quantity = quantity - (quantity % 0.1M);
                    }
                    if (stepSize.Value == 0.0001M)
                    {
                        quantity = quantity - (quantity % 0.0001M);
                    }
                    if (stepSize.Value == 0.00001M)
                    {
                        quantity = quantity - (quantity % 0.00001M);
                    }
                    if (stepSize.Value == 0.000001M)
                    {
                        quantity = quantity - (quantity % 0.000001M);
                    }
            ...
            if (notional < symbolData.MinNotionalFilter.MinNotional)
                //return BinanceTradeRuleResult.CreateFailed($"Market addition failed. Order size {notional} is less than minimal order size {symbolData.MinNotionalFilter.MinNotional}");
        ...
        public async Task<BinanceTradeRuleResult> GetClaimPirceAndQuantityAsync(string symbol, decimal quantity, decimal? price, OrderType type)
        {
            var outputQuantity = quantity;
            var outputPrice = price;
        ...
        public BinanceTradeRuleResult GetClaimPirceAndQuantity(string symbol, decimal quantity, decimal? price, OrderType type)
        {
            var outputQuantity = quantity;
            var outputPrice = price;
